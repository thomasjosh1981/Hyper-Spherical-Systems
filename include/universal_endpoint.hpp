...
//   OUTBOUND (seal):  ISSI compress → Homophonic scramble → AES-256
//   INBOUND  (unseal): AES-256 → deobfuscate → ISSI decompress
//
...
enum class PipelineStage { ISSI, HOMOPHONIC, AES256 };

struct PipelineConfig {
    // The 3 stages in the order they will be applied (outbound sealing)
    // Default: ISSI → HOMOPHONIC → AES256
...
public:
    ...
    // Swap the order of ISSI and Homophonic stages at runtime
    void swap_issi_homophonic_order();
