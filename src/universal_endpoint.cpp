...
//
// Bidirectional pipeline:  ISSI ↔ Homophonic ↔ AES-256  (order configurable)
...
void UniversalEndpoint::swap_issi_homophonic_order() {
    // Find ISSI and HOMOPHONIC positions and swap them
    int issi_pos = -1, hom_pos = -1;
    for (int i = 0; i < 3; ++i) {
        if (cfg_.stages[i] == PipelineStage::ISSI)      issi_pos = i;
        if (cfg_.stages[i] == PipelineStage::HOMOPHONIC) hom_pos   = i;
    }
    if (issi_pos >= 0 && hom_pos >= 0)
        std::swap(cfg_.stages[issi_pos], cfg_.stages[hom_pos]);
    std::cout << "[UE] Pipeline order swapped: "
              << (cfg_.stages[0] == PipelineStage::ISSI ? "ISSI" : "HOMOPHONIC")
              << " → "
              << (cfg_.stages[1] == PipelineStage::ISSI ? "ISSI" : "HOMOPHONIC")
              << " → AES256\n";
}
...
std::string UniversalEndpoint::apply_stage(const std::string& data,
    ...
    switch (stage) {
        case PipelineStage::ISSI: {
            std::cout << "[UE] Stage: ISSI Compression (" << data.size() << " bytes in)\n";
            ...
            for (const auto& e : entries) {
                if (e.is_compressed) {
                    ...
                    out += data.substr(e.context_pos, e.original_size_bytes);
                    out += ' ';
                }
            }
...
std::string UniversalEndpoint::reverse_stage(const std::string& data,
    ...
    switch (stage) {
        case PipelineStage::ISSI: {
            std::cout << "[UE] Reverse Stage: ISSI Decompress\n";
