"""bridge.py — ctypes wrapper around pirate_bridge.dll.
...
class TessEngine:
    ...
    DEFAULT_DLL = r"C:\Users\twist\workspace\project_tesseract\build\pirate_bridge.dll"
    ...
    def __init__(self, dll_path: Optional[str] = None) -> None:
        ...
        self._bind()
        handle = self._lib.pirate_create()
        if not handle:
    ...
    def _bind(self) -> None:
        ...
        lib.pirate_init_vram.argtypes = [ctypes.c_void_p,
                                       ctypes.c_uint64, ctypes.c_uint64]
        lib.pirate_init_vram.restype  = ctypes.c_int
        lib.pirate_compress.argtypes = [ctypes.c_void_p,
                                      ctypes.c_char_p, ctypes.c_int,
                                      ctypes.c_char_p, ctypes.c_int]
        lib.pirate_compress.restype  = ctypes.c_int
        lib.pirate_decompress.argtypes = [ctypes.c_void_p,
                                        ctypes.c_char_p, ctypes.c_int,
                                        ctypes.c_char_p, ctypes.c_int]
        lib.pirate_decompress.restype  = ctypes.c_int
        lib.pirate_push_layer.argtypes = [ctypes.c_void_p,
        ...
        lib.pirate_vram_used.argtypes = [ctypes.c_void_p]
        lib.pirate_vram_used.restype  = ctypes.c_uint64
        lib.pirate_vram_budget.argtypes = [ctypes.c_void_p]
        lib.pirate_vram_budget.restype  = ctypes.c_uint64
        lib.pirate_ram_staging_used.argtypes = [ctypes.c_void_p]
        lib.pirate_ram_staging_used.restype  = ctypes.c_uint64
        lib.pirate_ram_staging_limit.argtypes = [ctypes.c_void_p]
        lib.pirate_ram_staging_limit.restype  = ctypes.c_uint64
        ...
        lib.pirate_io_write.argtypes = [ctypes.c_void_p, ctypes.c_char_p,
                                      ctypes.c_char_p, ctypes.c_int]
        lib.pirate_io_write.restype  = ctypes.c_int
        lib.pirate_io_read.argtypes = [ctypes.c_void_p, ctypes.c_char_p,
                                     ctypes.c_char_p, ctypes.c_int]
        lib.pirate_io_read.restype  = ctypes.c_int
        lib.pirate_checkpoint_save.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
        lib.pirate_checkpoint_save.restype  = ctypes.c_int
        lib.pirate_checkpoint_load.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
        lib.pirate_checkpoint_load.restype  = ctypes.c_int
        lib.pirate_index_build.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
        lib.pirate_index_build.restype  = ctypes.c_int
    ...
    def _last_error_str(self, h=None) -> str:
        ...
        try:
            return self._lib.pirate_last_error(h).decode(errors="replace")
        except Exception:
            return "<unknown>"
    ...
    def close(self) -> None:
        if getattr(self, "_h", None):
            self._lib.pirate_destroy(self._h)
            self._h = None
    ...
    def compress(self, text: str) -> CompressResult:
        ...
        n = self._lib.pirate_compress(self._h, encoded, len(encoded),
                                    out, len(out))
    ...
    def decompress(self, text: str) -> str:
        ...
        n = self._lib.pirate_decompress(self._h, encoded, len(encoded),
                                      out, len(out))
    ...
    def push_layer(self, layer_id: int, byte_size: int) -> int:
        return self._check(
            self._lib.pirate_push_layer(self._h,
                                      ctypes.c_uint32(layer_id),
                                      ctypes.c_uint64(byte_size)),
            "push_layer")

    def vram_usage_pct(self) -> float:
        return float(self._lib.pirate_vram_usage_pct(self._h))
    ...
    def vram_used(self) -> int:
        return int(self._lib.pirate_vram_used(self._h))

    def vram_budget(self) -> int:
        return int(self._lib.pirate_vram_budget(self._h))

    def ram_staging_used(self) -> int:
        return int(self._lib.pirate_ram_staging_used(self._h))

    def ram_staging_limit(self) -> int:
        return int(self._lib.pirate_ram_staging_limit(self._h))

    def observe_layer(self, layer_id: int) -> None:
        self._lib.pirate_observe_layer(self._h, ctypes.c_uint32(layer_id))

    def predict_next(self, n: int = 4) -> tuple[list[int], float]:
        ids = (ctypes.c_uint32 * n)()
        conf = ctypes.c_float()
        count = self._lib.pirate_predict_next(self._h, ids, n, ctypes.byref(conf))
        return [int(ids[i]) for i in range(count)], float(conf.value)

    def total_observations(self) -> int:
        return int(self._lib.pirate_total_observations(self._h))

    def io_write(self, rel_path: str, data: bytes) -> int:
        return self._check(
            self._lib.pirate_io_write(self._h, rel_path.encode(),
                                    data, len(data)),
            "io_write")

    def io_read(self, rel_path: str, cap: int = 1 << 20) -> bytes:
        buf = ctypes.create_string_buffer(cap)
        n = self._lib.pirate_io_read(self._h, rel_path.encode(),
                                   buf, cap)
        self._check(1 if n <= 0 else 0, "io_read")
        return buf.raw[:n]

    def checkpoint_save(self, path: str) -> None:
        self._check(self._lib.pirate_checkpoint_save(self._h, path.encode()),
                    "checkpoint_save")

    def checkpoint_load(self, path: str) -> None:
        self._check(self._lib.pirate_checkpoint_load(self._h, path.encode()),
                    "checkpoint_load")

    def index_build(self, directory: str) -> None:
        self._check(self._lib.pirate_index_build(self._h, directory.encode()),
                    "index_build")
