"""pirate_gui — top-level entry point.

Usage:
    python -m pirate_gui                  # first-run wizard → dashboard
    python -m pirate_gui --reset-config   # force the wizard to run again
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="pirate_gui")
    ...
    parser.add_argument("--engine-path", default=None,
                        help="Override path to pirate_bridge.dll")
    args = parser.parse_args(argv)

    app = QtWidgets.QApplication(sys.argv)
    app.setApplicationName("Pirate Llama Control Center")
    app.setStyle("Fusion")
