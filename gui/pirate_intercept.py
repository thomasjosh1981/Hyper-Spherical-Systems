...
MODE_BOTH   = "both"    # ISSI + 10x (recommended — synergistic, not additive)
MODE_ISSI   = "issi"    # ISSI dict substitution only
MODE_TENX   = "tenx"    # 10x semantic strip only
MODE_DENY   = "denied"  # No optimization

# Per-app mode cache  app_name → MODE_*
_app_mode_cache: Dict[str, str] = {}
APP_MODE_FILE = Path.home() / ".hypes" / "app_modes.json"


def _load_app_modes() -> Dict[str, str]:
    try:
        if APP_MODE_FILE.exists():
            return json.loads(APP_MODE_FILE.read_text(encoding="utf-8"))
    except Exception:
        pass
    return {}


def _save_app_mode(app_name: str, mode: str) -> None:
    try:
        APP_MODE_FILE.parent.mkdir(parents=True, exist_ok=True)
        data = _load_app_modes()
        data[app_name] = mode
        APP_MODE_FILE.write_text(json.dumps(data, indent=2), encoding="utf-8")
    except Exception:
        pass


def get_app_mode(app_name: str) -> str:
    """Return persisted compression mode for app, or None if undecided."""
    if not _app_mode_cache:
        _app_mode_cache.update(_load_app_modes())
    return _app_mode_cache.get(app_name, "undecided")


def set_app_mode(app_name: str, mode: str) -> None:
    _app_mode_cache[app_name] = mode
    _save_app_mode(app_name, mode)
...
def _request_app_consent_gui(app_name: str, llm_name: str = "", llm_url: str = "") -> str:
    """
    Show the HypeS 4-mode routing consent popup for a newly detected app.
    Returns one of: MODE_BOTH | MODE_ISSI | MODE_TENX | MODE_DENY
    """
    _WHY_BOTH = (
        "<b>Why Both works best:</b><br>"
        "ISSI and 10x are <i>complementary layers</i>, not alternatives.<br>"
        "&#9658; <b>ISSI</b> compresses at the <i>dictionary level</i> — domain phrases become "
        "single tokens ({H1}, {API1} …). Works best on <i>structured / repeated content</i>.<br>"
        "&#9658; <b>10x Semantic Strip</b> works at the <i>sentence level</i> — removes filler, "
        "politeness, hedging, and glue words the LLM doesn't need. Works best on <i>conversational "
        "/ natural language</i>.<br><br>"
        "<b>Why one alone underperforms:</b><br>"
        "ISSI alone leaves all the filler words intact — only compresses the nouns.<br>"
        "10x alone leaves all the repeated domain phrases as full words — only compresses the glue.<br><br>"
        "<b>Together</b>: 10x strips the sentence first, then ISSI compresses what remains. "
        "Combined ratio is <b>2.5x–4x</b>. Either alone is ~1.3–1.6x."
    )

    llm_display = llm_name or "Unknown LLM"
    url_display  = llm_url  or "(local)"
    ...
    try:
        ...
        chosen_mode = [MODE_DENY]   # mutable cell captured by lambdas

        dlg = QtWidgets.QDialog()
        dlg.setWindowTitle("HypeS — New LLM Traffic Detected")
        ...
        dlg.setStyleSheet("""
            ...
            QLabel  { background: transparent; border: none;
                      color: #8899bb; font-family:'Segoe UI','Inter',sans-serif; }
            QCheckBox { color:#5a7a9a; font-size:11px; }
        ...
        hdr.setStyleSheet(
            ...
            "stop:0 rgba(99,102,241,0.18),stop:1 rgba(168,85,247,0.10));"
            "border-radius:9px; border:1px solid rgba(99,102,241,0.30);"
        )
        ...
        eyebrow = QtWidgets.QLabel("▶▶  hyper-spherical systems — llm traffic detected")
        eyebrow.setStyleSheet("color:#6366f1; font-size:9px; letter-spacing:0.16em; font-weight:700;")
        headline = QtWidgets.QLabel(f"{app_name}  →  {llm_display}")
        headline.setStyleSheet("color:#f0f4ff; font-size:15px; font-weight:800;")
        urlline = QtWidgets.QLabel(url_display)
        urlline.setStyleSheet("color:#4a6a8a; font-size:10px; font-family:monospace;")
        txt.addWidget(eyebrow)
        txt.addWidget(headline)
        txt.addWidget(urlline)
        hlay.addWidget(ico)
        hlay.addSpacing(12)
        hlay.addLayout(txt)
        hlay.addStretch()
        ...
        why_box = QtWidgets.QLabel(_WHY_BOTH)
        why_box.setWordWrap(True)
        why_box.setTextFormat(QtCore.Qt.RichText)
        why_box.setStyleSheet(
            "background:rgba(99,102,241,0.07); border:1px solid rgba(99,102,241,0.20);"
            "border-radius:8px; padding:12px 14px; color:#7a9abb; font-size:11px;"
        )
        ...
        root.addWidget(perm_chk)
        root.addSpacing(16)

        # ── 4-button row ──────────────────────────────────────────────────────
        BTN_BASE = (
            "QPushButton { border-radius:8px; padding:8px 18px;"
            " font-size:12px; font-weight:700; border:none; min-height:38px; }"
        )
        btn_row = QtWidgets.QHBoxLayout()
        btn_row.setSpacing(10)

        def _make_btn(label, tip, grad_stops, mode_val):
            b = QtWidgets.QPushButton(label)
            b.setToolTip(tip)
            b.setStyleSheet(
                BTN_BASE +
                f"QPushButton {{ background:qlineargradient(x1:0,y1:0,x2:1,y2:0,{grad_stops});"
                f" color:#ffffff; }}"
                f"QPushButton:hover {{ opacity:0.85; }}"
            )
            def _click(checked=False, m=mode_val):
                chosen_mode[0] = m
                dlg.accept()
        ...
        btn_both = _make_btn(
            "⚡ Both (recommended)",
            "ISSI dict compression + 10x semantic strip — 2.5–4x combined ratio",
            "stop:0 #4f46e5, stop:1 #7c3aed",
            MODE_BOTH,
        )
        btn_issi = _make_btn(
            "ISSI only",
            "Dictionary n-gram substitution only — ~1.5x ratio. Best for structured/code prompts.",
            "stop:0 #0e7490, stop:1 #0369a1",
            MODE_ISSI,
        )
        btn_tenx = _make_btn(
            "10x strip only",
            "Semantic filler/glue removal only — ~1.3x ratio. Best for chat/natural language.",
            "stop:0 #065f46, stop:1 #047857",
            MODE_TENX,
        )
        btn_no = QtWidgets.QPushButton("No thanks")
        btn_no.setStyleSheet(
            BTN_BASE +
            "QPushButton { background:rgba(30,40,60,0.85); color:#4a6a8a;"
            " border:1px solid rgba(255,255,255,0.10); }"
            "QPushButton:hover { color:#ff7777; border-color:rgba(255,80,80,0.40); }"
        )
        btn_no.clicked.connect(dlg.reject)

        btn_row.addWidget(btn_both)
        btn_row.addWidget(btn_issi)
        btn_row.addWidget(btn_tenx)
        btn_row.addStretch()
        ...
        decision = MODE_DENY if mode == MODE_DENY else "allowed"
        if perm_chk.isChecked():
            _set_app_consent(app_name, decision)
            set_app_mode(app_name, mode)

        return mode

    except Exception as _qt_err:
        # ── Headless fallback ─────────────────────────────────────────────────
        print(f"\n[HypeS] {app_name} is sending prompts to {llm_display} at {url_display}")
        print("Route through HypeS compression?")
        print("  1) Both ISSI + 10x strip  (recommended — 2.5-4x combined)")
        print("  2) ISSI dict only          (~1.5x — good for structured/code)")
        print("  3) 10x strip only          (~1.3x — good for chat/natural language)")
        print("  4) No thanks")
        print()
        print("Why Both is best: ISSI compresses domain nouns; 10x strips sentence filler.")
        print("Either alone only catches half the redundancy. Together they're synergistic.")
        ans = input("Choice [1/2/3/4, default=1]: ").strip() or "1"
        mode_map = {"1": MODE_BOTH, "2": MODE_ISSI, "3": MODE_TENX, "4": MODE_DENY}
        mode = mode_map.get(ans, MODE_BOTH)
        if mode != MODE_DENY:
            _set_app_consent(app_name, "allowed")
        set_app_mode(app_name, mode)
        return mode
...
def _ensure_app_consent(app_name: str, llm_name: str = "", llm_url: str = "") -> str:
    """
    Ensure the user has chosen a compression mode for this app.
    Returns the chosen MODE_* constant.
    Thread-safe: only one dialog per app shown at a time.
    """
    existing_mode = get_app_mode(app_name)
    if existing_mode != "undecided":
    ...
    with _consent_pending_lock:
        if app_name in _consent_pending:
            ...
            mode = _request_app_consent_gui(app_name, llm_name=llm_name, llm_url=llm_url)
            event.set()
            with _consent_pending_lock:
                _consent_pending.pop(app_name, None)
            return mode

    # Another thread already showing dialog — wait
    event.wait(timeout=60)
    return get_app_mode(app_name)
...
def _compress_body(body_json: dict, fmt: str,
                   ...
                   llm_name: str = "", llm_url: str = ""
                   ) -> Tuple[dict, int, int]:
    """
    Compress all text fields in a request body through the ISSI unified engine.
    Respects per-app compression mode (Both / ISSI / 10x / Deny).
    ...
    # Get (or prompt for) per-app compression mode
    mode = _ensure_app_consent(app_name, llm_name=llm_name or provider, llm_url=llm_url)
    if mode == MODE_DENY:
        return body_json, 0, 0
    ...
    for field in fields:
        ...
        if not text or len(text.strip()) < 8:
            continue
        try:
            # Only compress user-role messages; pass system prompts through (they contain ISSI dict)
            role = field.get("role", "user")
            if role == "system":
                continue
            compressed, ratio = _issi_compress(text, mode=mode)
            orig_tok = max(1, len(text.split()) * 4 // 3)
    ...
    _log(f"[ISSI:{mode}] {app_name} → {provider}/{model}: {orig_total}tok → {comp_total}tok")
...
def _push_hud_stats(orig_tokens: int, comp_tokens: int, model: str = "gpt-4o",
                    url: str = "", app_name: str = "") -> None:
    """Push token savings to the floating Token HUD (non-blocking, multi-path).
    Now includes URL, model, app identity for the upgraded HUD display.
    Also appends to a persistent rolling log (rotated every 24h).
    """
    if orig_tokens <= 0:
        return
    import time as _time
    _seq = int(_time.time() * 1000) & 0x7FFFFFFF
    stat_record = {
        "seq":        _seq,
        "pre_tokens": orig_tokens,
        "post_tokens": comp_tokens,
    ...
    # ── Path 1: write shared stat file (picked up by LAUNCH_TOKEN_HUD.py) ──
    try:
        _hud_live = Path.home() / ".hypes" / "hud_live.json"
        _hud_live.parent.mkdir(parents=True, exist_ok=True)
        _hud_live.write_text(
            __import__("json").dumps(stat_record),
            encoding="utf-8",
        )
    except Exception:
        pass
    # ── Path 1b: append to persistent 24h rolling log ──
    try:
        _log_dir = Path.home() / ".hypes" / "token_logs"
        _log_dir.mkdir(parents=True, exist_ok=True)
        _day_file = _log_dir / f"{_time.strftime('%Y-%m-%d')}.jsonl"
        with open(_day_file, "a", encoding="utf-8") as f:
            f.write(__import__("json").dumps(stat_record) + "\n")
    except Exception:
        pass
    # ── Path 2: in-process Qt signal (if HUD is running in same process) ──
    try:
        sys.path.insert(0, str(Path(__file__).parent / "pirate_gui"))
        from pirate_gui.token_hud import push_compression_stat
        push_compression_stat(orig_tokens, comp_tokens, model=model)
    except Exception:
        pass
...
def _issi_compress(text: str, mode: str = MODE_BOTH) -> Tuple[str, float]:
    """
    Compress text through ISSI unified engine.
    mode: MODE_BOTH | MODE_ISSI | MODE_TENX
    """
    try:
        sys.path.insert(0, str(Path(__file__).parent))
        from issi_engine import get_registry, semantic_strip, prune_text, count_tokens
        reg = get_registry()
        orig_tok = count_tokens(text)

        if mode == MODE_BOTH:
            # Full pipeline: phrase strip → prune → ISSI dict
            stripped, _, _ = semantic_strip(text)
            pruned = prune_text(stripped)['optimized_text']
            result = reg.issi.compress(pruned)
        elif mode == MODE_ISSI:
            # ISSI dict only (no semantic strip)
            pruned = prune_text(text.upper())['optimized_text']
            result = reg.issi.compress(pruned)
        elif mode == MODE_TENX:
            # Semantic strip only — preserves readability
            result, _, _ = semantic_strip(text)
        else:
            return text, 1.0

        comp_tok = count_tokens(result)
        ratio = orig_tok / max(1, comp_tok)
        return result, ratio
    except Exception:
        pass
    # Final fallback: phrase table only
    result = text
...
def _forward_cloud_request(parsed: dict) -> bytes:
    ...
    try:
        ...
        if orig_tokens > 0:
            _cloud_url = f"https://{host}{parsed.get('path', '/v1/chat/completions')}"
            threading.Thread(
                target=_push_hud_stats,
                args=(orig_tokens, comp_tokens),
                kwargs={"model": model, "url": _cloud_url, "app_name": app_name},
                daemon=True,
            ).start()
...
def _handle_client(client_sock: socket.socket, intercept_port: int) -> None:
    try:
        ...
        if orig_tokens > 0:
            _local_url = f"http://{backend[0]}:{backend[1]}{parsed.get('path', '/v1/chat/completions')}"
            threading.Thread(
                target=_push_hud_stats,
                args=(orig_tokens, comp_tokens),
                kwargs={"model": model, "url": _local_url, "app_name": app_name},
                daemon=True,
            ).start()
